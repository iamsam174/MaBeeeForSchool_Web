...
- **多様な動作モード:** 利用者の障害特性や操作の習熟度に合わせて、以下の4つの動作モードを切り替えられます。
  ...
  2. **マウスオーバー（マウスポインタがボタンに触れた瞬間）でタイマー実行モード:** マウスポインタがボタンに触れた瞬間にタイマーが起動し、一定時間MaBeee for SchoolがONになります。
  3. **マウスポインタがボタン内にある間マビーONモード:** マウスポインタがボタンの領域内にある間だけMaBeee for SchoolがONになり続け、マウスポインタがボタンの外に出るとOFFになります。
...
12. MaBeee for Schoolとの接続を終わらせるには、使用している端末のブラウザで開いているページをリロードするかタブを削除します。その後、スマートフォン・タブレット端末・Chromebookは、アプリスイッチャーでブラウザを終了させてください。
